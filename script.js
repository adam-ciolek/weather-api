...
class fetchForecastApi {   
    ...
    getLocation(query, callback) {
        $.getJSON(this.searchUrl, {query}).done(data => callback(data)).fail(() => callback(null))
    }
...
class dataMiddleware {
    ...
    prepareDataFroDom(data) {
        console.log(data)
        const {predictability,humidity,wind_speed,max_temp,min_temp,air_pressure,applicable_date,the_temp,weather_state_abbr,weather_state_name} = data.consolidated_weather[0]
        const todaysForecastGeneral = this.gatherTodaysForecastGeneral({
            applicable_date,the_temp,weather_state_abbr,weather_state_name, title: data.title,
        })
        const todaysForecastDetails = this.gatherTodaysForecastdetails({
            predictability,humidity,wind_speed,max_temp,min_temp,air_pressure
        })
       
        this.prepareTodaysForecastDetails(todaysForecastDetails);
        this.displayForecast.showTodaysForecast(todaysForecastGeneral);
        this.prepareUpcomingDays(data.consolidated_weather);
    ...
    fetchWeather(query) {
        this.fetchForecastApi.getLocation(query, location => {
            ...
            this.fetchForecastApi.getWeatherData(location[0].woeid, data => {
                ...
                this.dataMiddleware.prepareDataFroDom(data);
            })
        });
    }
